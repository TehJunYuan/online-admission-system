    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 mt-12">
            ...
            <div class="flex p-4 mb-4 text-sm text-yellow-800 border border-yellow-300 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300 dark:border-yellow-800" role="alert">
                ...
                <div>
                    {{ __('admin.info_1') }}<span class="font-bold">{{ __('admin.info_4') }}</span> {{ __('admin.info_3') }}<span class="font-bold">{{ __('admin.info_4') }}</span> 
                </div>
            </div>
            {{-- end alert --}}
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4">
                                    @if ($pending_verified_candidate->getCandidate_profile_status_id())
                                        <span class="bg-yellow-100 text-yellow-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full dark:bg-yellow-900 dark:text-yellow-300">{{  __('Complete check application detail, pending AARO') }}</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-center flex flex-col">
                                    <a href="{{  route('approve.AARO', ['id' => Crypt::encrypt($pending_verified_candidate->getApplication_record_id())])  }}" class="font-medium text-green-600 dark:text-green-500 hover:underline my-1">{{ __('Approve') }}</a>
                                    <a href="{{ route('reject.home', ['id' => Crypt::encrypt($pending_verified_candidate->getApplication_record_id())]) }}" class="font-medium text-red-600 dark:text-red-500 hover:underline my-1" onclick="window.open(this.href, '_blank', 'width=500,height=700'); return false;">{{ __('Reject') }}</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
