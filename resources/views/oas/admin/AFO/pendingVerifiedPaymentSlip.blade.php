<x-app-layout>
    @include('layouts.afoNavigation')

    <div class="p-4 sm:ml-64">
        <div class="p-4 mt-12">
            {{-- header --}}
            <h2 class="mx-2 my-4 text-3xl font-extrabold leading-none tracking-tight text-gray-900 md:text-4xl dark:text-white">{{ __('admin.verify_payment') }}</h2>
            {{-- end header --}}
            ...
            <div class="flex p-4 mb-4 text-sm text-yellow-800 border border-yellow-300 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300 dark:border-yellow-800" role="alert">
                ...
                <div>
                    {{ __('admin.info_1_AFO') }}<span class="font-bold">{{ __('admin.info_2_AFO') }}</span> {{ __('admin.info_3_AFO') }} <span class="font-bold">{{ __('admin.info_4_AFO') }}</span> {{ __('admin.info_5_AFO') }} <span class="font-bold">{{ __('admin.info_6_AFO') }}</span> {{ __('admin.info_7_AFO') }} <span class="font-bold">{{ __('admin.info_8_AFO') }}</span> 
                </div>
            </div>
            {{-- end alert --}}
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4">
                                    @if ($pending_verified_candidate->getCandidate_profile_status_id())
                                        <span class="bg-yellow-100 text-yellow-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded-full dark:bg-yellow-900 dark:text-yellow-300">{{  __('Pending verify by AFO') }}</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <a href="{{ route('applicationForm.getPayment',['id'=> Crypt::encrypt($pending_verified_candidate->getApplication_record_id())]) }}" target="_blank" class="text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-1 text-center inline-flex items-center mr-2 dark:bg-yellow-500 dark:hover:bg-yellow-600 dark:focus:ring-yellow-400">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                ...
                                <td class="px-6 py-4">
                                    <a href="{{ route('remark.home', ['id' => Crypt::encrypt($pending_verified_candidate->getApplication_record_id())]) }}" target="_blank" class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-1 text-center inline-flex items-center mr-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" onclick="window.open(this.href, '_blank', 'width=500,height=500'); return false;">
                                        ...
                                        &nbsp;
                                        {{ __('Write your messages here') }}
                                    </a>
                                </td>
                                <td class="px-6 py-4 text-center flex flex-col">
                                    <a href="{{  route('approve.AFO', ['id' => Crypt::encrypt($pending_verified_candidate->getApplication_record_id())])  }}" class="font-medium text-green-600 dark:text-green-500 hover:underline my-1">{{ __('button.approve') }}</a>
