<x-app-layout>
       ...
       <div class="p-4 mt-12">
            ...
            <nav class="flex mb-4" aria-label="Breadcrumb">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                    <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                    <span class="ml-1 text-sm font-medium text-gray-700 md:ml-2 dark:text-white">{{ __('stepper.apply_edit_step_2') }}</span>
                    </div>
                </li>
                </ol>
            </nav>
            <h2 class="mb-4 text-3xl font-extrabold leading-none tracking-tight text-gray-900 md:text-4xl dark:text-white">{{ __('stepper.apply_edit_step_2') }}</h2>
            {{-- end header --}}
            {{-- stepper --}}
            ...
            {{-- form --}}
            <form action="{{ route('academicDetail.update',['id'=>Crypt::encrypt($APPLICATION_RECORD_ID)]) }}" method="post" class="mt-6" enctype="multipart/form-data" onsubmit="return validateInput();">
                @csrf
                <!-- component -->
                ...
                @for ($i = 0; $i < count($data['schoolLevels']); $i++)
                    <div class="grid md:grid-cols-3 px-4 py-5 text-sm text-gray-700 border-b border-gray-200 gap-x-16 dark:border-gray-700">
                        <div class="text-gray-500 dark:text-gray-400">{{ $data['schoolLevels'][$i]->name }}</div>
                        <input type="hidden" name="school_level_id[]" value="{{ $data['schoolLevels'][$i]->id }}">
                        @if ($data['academicRecords'][$i]->status == 1)
                            <div class="my-1">
                                <input type="text" id="school_name[{{ $data['schoolLevels'][$i]->id }}]" name="school_name[]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Name of institution" onkeyup="if (/[^|A-Za-z0-9\s/]+/g.test(this.value)) this.value = this.value.replace(/[^|A-Za-z0-9\s/]+/g,'')"  maxlength="50" value="{{ $data['academicRecords'][$i]->school_name }}">
                            </div>
                            <div class="my-1">
                                <input type="text" id="school_graduation[{{ $data['schoolLevels'][$i]->id }}]" name="school_graduation[]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Year" onkeyup="if (/[^|0-9]+/g.test(this.value)) this.value = this.value.replace(/[^|0-9]+/g,'')" minlength="4" maxlength="4" value="{{ $data['academicRecords'][$i]->school_graduation }}">
                            </div>
                        @else 
                            <div class="my-1">
                ...
                <a href="{{route('statusOfHealth.home',['id' => Crypt::encrypt($APPLICATION_RECORD_ID)])}}" class="mt-4 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">{{ __('button.next') }}</a>
            </form>
            {{-- end form --}}
       </div>
    </div>
    <script>
        function validateInput(){
            var total_level = {!! count($data['schoolLevels']) !!};
