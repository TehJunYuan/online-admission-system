       ...
       <div class="p-4 mt-12">
            ...
            <nav class="flex mb-4" aria-label="Breadcrumb">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                    <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                    <span class="ml-1 text-sm font-medium text-gray-700 md:ml-2 dark:text-white">{{ __('stepper.apply_step_3') }}</span>
                    </div>
                </li>
                </ol>
            </nav>
            <h2 class="mb-4 text-3xl font-extrabold leading-none tracking-tight text-gray-900 md:text-4xl dark:text-white">{{ __('stepper.apply_step_3') }}</h2>
            {{-- end header --}}
            {{-- stepper --}}
            ...
            {{-- form --}}
            <form action="{{route('statusOfHealth.create',['id' => Crypt::encrypt($APPLICATION_RECORD_ID)])}}" method="post" class="mt-6" enctype="multipart/form-data">
                @csrf
                ...
                @for ($i = 0; $i < count($get_all_active_diseases); $i++)  
                    <div class="grid md:grid-cols-3 px-4 py-5 text-sm text-gray-700 border-b border-gray-200 gap-x-16 dark:border-gray-700">
                        <div class="text-gray-500 dark:text-gray-400">{{ $get_all_active_diseases[$i]->name }}</div>
                        <input type="hidden" name="disease_id[]" value="{{ $get_all_active_diseases[$i]->id }}">
                        <div class="my-1">
                            <select name="disease_status[]" id="disease_status[{{ $get_all_active_diseases[$i]->id }}]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" onchange="setRequired(this,'disease_remark[{{ $get_all_active_diseases[$i]->id }}]')">
                                <option value="0" selected>{{ __('No, I don\'t have') }}</option>
                                <option value="1">{{ __('Yes') }}</option>
                            </select>
                        </div>
                        <div class="my-1">
                            <textarea name="disease_remark[]" id="disease_remark[{{ $get_all_active_diseases[$i]->id }}]" rows="1" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Write your thoughts here..."></textarea>
                        </div>
                    </div>
                @endfor
